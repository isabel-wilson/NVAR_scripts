...
stc = mne.read_source_estimate(path + subject)
F = stc.lh_data
...
opt = {
    ...
    "WinLength": 2, # STFT window length # CHANGED
...
# Write to file
with open(f"{path}output/sprint_output.txt", "w") as file:
    file.write(output)
...
fgs_noout = [SPRiNT_remove_outliers(fgs[i], output['ts'], opt) for i in range(len(fgs))]
# Write to file
for i in range(len(fgs_noout)): 
    df_noout = fgs_noout[i].to_df(bands)
    df_noout.to_csv(f"{path}output/foof_nooutliers_{subject}_vertex_{str(i)}.csv", na_rep="NaN")
